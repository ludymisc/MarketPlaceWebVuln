...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await res.json({ message: "register berhasil"});
      console.log({ email, password });

      if (res.ok) {
        console.log("REGISTER SUKSES:", data.message);
        navigate('/login');
      } else {
        console.log("REGISTER ERROR:", data.message || "Gagal mendaftar");
        ...
        <div className="w-full max-w-md px-4">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-xs text-gray-400">
              Selamat datang! Silakan buat akun atau masuk ke akun yang sudah ada untuk melanjutkan.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <p className="text-center text-xs text-gray-500 mt-6">
              Sudah punya akun?{' '}
              <a href="/login" className="font-bold text-[#16A085] hover:underline">
                Masuk
              </a>
            </p>
